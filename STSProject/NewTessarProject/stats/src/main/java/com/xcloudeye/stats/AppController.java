...
//			appRebuildLogic.getAdvanceDao().closeMongoClient();
	...
	String AppPayChSort(@RequestParam String app,@RequestParam String start,@RequestParam String end,HttpSession session) throws MapperException{
		...
		logger.info("channels list:" + channels);
...
//		int cDate = Integer.valueOf(1453690800);
    ...
    public @ResponseBody
    String RoiQuery(@RequestParam String app, @RequestParam String start,
            @RequestParam String end) throws MapperException {
        int startTime = Integer.valueOf(start);
        int endTime = Integer.valueOf(end);
        int appid = Integer.valueOf(app);
        StringBuilder result = new StringBuilder();
        userInfoLogic.initPaymentLogic();
        List<UserInfo> list = userInfoLogic.readMoreByQuery(startTime, endTime);
        
        List<UserInfo> disList = new ArrayList<>();
        Map<String, UserInfo> map = new HashMap<>();
        for (UserInfo u : list) {
            if (map.get(u.getUid()) == null) {
                disList.add(u);
                map.put(u.getUid(), u);
            }
        }
        disList.forEach(user -> {
            logger.info("newUserList:" + user.getUid());
        });
        
        paymentLogic.initPaymentLogic();
        PaySortChOut geData = paymentLogic.getPaySortByNewUser(disList, startTime, endTime);
        geData.setApp(app);
        geData.setApi("roi_query");
        JSONValue jsonValue = JSONMapper.toJSON(geData);
        String jsonStr = jsonValue.render(true);
        result.append("apiStatus").append('(').append(jsonStr).append(')');
        return result.toString();
