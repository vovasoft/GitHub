 ...
 *
 */
public class AngelsFifteenMinuteSchedule
{
  ...
  private static String AG_NAME_ID = "7";
    private static final Logger logger = LoggerFactory.getLogger(AngelsFifteenMinuteSchedule.class);
...
//      ApplicationContext context = new ClassPathXmlApplicationContext("/hoursch-conf.xml");
  ...
  public void getAGQuartzByFifMinutes (){
  	try{
  	  DateUtil.setBSCurrentTimeMinuteSecond();
        int start=DateUtil.getBscurrentMinuteTime();
      System.out.println("AG fifteen quart start..."+DateUtil.getBscurrentMinuteTime());
      //初始化Dao操作参数
      initApp();
      
      //需要每15分钟计算一次的接口 接口数据
      FifteenMinuteLogic fifteenLogic = new FifteenMinuteLogic(jedis, client, mongoDb, client_user,mongoDb_user, jedisPool, mAppMap.get(AG_NAME_ID));
      fifteenLogic.insertData();
      
      System.out.println("AG fifteen quart end..."+DateUtil.getBscurrentMinuteTime());
  	}catch(Exception e){
  		e.printStackTrace();
  	}finally{
  		jedis.close();
