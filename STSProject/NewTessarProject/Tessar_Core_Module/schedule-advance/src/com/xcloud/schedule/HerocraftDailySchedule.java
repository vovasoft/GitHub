 ...
 *
 */
public class HerocraftDailySchedule
{	
	...
	private static final Logger logger = LoggerFactory.getLogger(HerocraftDailySchedule.class);
   ...
   public static void main( String[] args )
    {
	 ...
	 /**
	  ...
	  public void getHCQuartzByDaily (){
	  	try{
	  	  DateUtil.setBSCurrentTimeSecond();
			int start=DateUtil.getCurrentHourTimeSecond();
	      System.out.println("herocraft daily quart start..."+DateUtil.getBscurrentTime());
	      //初始化Dao操作参数
	      initApp();
	      //计算一天的接口 接口数据
			System.out.println("dailyLogic method is invoking....");
	      DailyLogic dailyLogic = new DailyLogic(jedis, client, mongoDb,client_user,mongoDb_user, jedisPool, mAppMap.get(HC_NAME_ID));
	      dailyLogic.insertData();

			//计算retention一天的数据接口 接口数据
			System.out.println("retentionLogic method is invoking....");
			RetentionLogic retentionLogic = new RetentionLogic(jedis, client, mongoDb,client_user,mongoDb_user, jedisPool, mAppMap.get(HC_NAME_ID));
			retentionLogic.insertData();

	      //app_trend_generic 接口数据
	      System.out.println("atgLogic method is invoking....");
		  AppTrendGenericLogic atgLogic = new AppTrendGenericLogic(jedis, client, mongoDb, jedisPool, mAppMap.get(HC_NAME_ID));
	      atgLogic.insertData();

			//pay_hobby_generic 接口
		  System.out.println("phgLogic method is invoking.... ");
		  PayHobbyGenericLogic phgLogic = new PayHobbyGenericLogic(jedis, client, mongoDb, jedisPool, mAppMap.get(HC_NAME_ID));
		  phgLogic.insertData();

	      System.out.println("herocraft daily quart end..."+DateUtil.getCurrentHourTimeSecond());
