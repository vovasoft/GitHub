...
public class SingleAngelsMongoConf {

	private static final SingleAngelsMongoConf singleAngelsMongoConf = new SingleAngelsMongoConf();
	...
	private SingleAngelsMongoConf() {
		Element element = this.loadXmlElement(GetConfigUtil.configFile);
		initMongoClient(element);
	}

	
	public synchronized static SingleAngelsMongoConf getInstance(){
        return singleAngelsMongoConf;
    }
	...
	private void initMongoClient(Element el) {
		...
		try {
			...
			String name = null;
			String dburi = null;
			Iterator i = foo.elementIterator("angelsdatabase");
