 ...
 */
public class HerocraftFiveMinuteSchedule {
    ...
    private static final Logger logger = LoggerFactory.getLogger(HerocraftFiveMinuteSchedule.class);
  @SuppressWarnings("resource")
  public static void main( String[] args )
  {
     //ApplicationContext context = new ClassPathXmlApplicationContext("/hoursch-conf.xml");
      HerocraftFiveMinuteSchedule test = new HerocraftFiveMinuteSchedule();
		test.getHCQuartzByFiveMinutes();
  }
    ...
    public void getHCQuartzByFiveMinutes() {
        try {
            DateUtil.setBSCurrentTimeMinuteSecond();
            Integer start=DateUtil.getBscurrentMinuteTime();
            System.out.println("hc five quart start..." + DateUtil.getBscurrentMinuteTime());
            //初始化Dao操作参数
            initApp();

            //需要每5分钟计算一次的接口 接口数据(五分钟去重数据)
            FiveMinuteLogic fiveLogic = new FiveMinuteLogic(jedis, client, mongoDb, client_user, mongoDb_user, jedisPool, mAppMap.get(HC_NAME_ID));
            fiveLogic.insertData();

            //需要每5分钟计算一次的接口 接口数据(五分钟新注册用户)
            FiveNewUserLogic fiveNewUser = new FiveNewUserLogic(jedis, client, mongoDb, client_user, mongoDb_user, jedisPool, mAppMap.get(HC_NAME_ID));
            fiveNewUser.insertNewUserData();

            System.out.println("hc five quart end..." + DateUtil.getBscurrentMinuteTime());
