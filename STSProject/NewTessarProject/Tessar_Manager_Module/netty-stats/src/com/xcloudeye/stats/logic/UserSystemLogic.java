...
import com.xcloudeye.stats.UserController;
...
public class UserSystemLogic {
    ...
    private static ClassPathXmlApplicationContext ctx = new ClassPathXmlApplicationContext("META-INF/redis-conf.xml");
     ...
     *             session 记录用户登录的session
     * @return callback回调信息
     */
    public UserCallBack getUserLogin(User user) {

        UserCallBack registBack = new UserCallBack(user.getUsername(), null, null, DateUtil.getCurrentTimeMinute(), LOGIN_FAILED, null);
        StringBuilder result = new StringBuilder();
        if (user.getUsername() == null || user.getUsername().equals(null) || "".equals(user.getUsername()) || user.getUsername() == "") {
            registBack.setFlag(LOGIN_USER_IS_EMPTY);
        } else if (user.getPassword() == null || user.getPassword().equals(null) || user.getPassword() == "" || "".equals(user.getPassword())) {
...
//			if (false) {
                ...
                if (userLogin(user)) {
                    ...
                    registBack.setEmail(this.getDbUser().getEmail());
                    registBack.setSkype(this.getDbUser().getSkype());
                    registBack.setSessionid(this.getDbUser().getUsername());
                    System.out.println("user.getUsername()---:" + user.getUsername());
                    System.out.println("getUser(user.getUsername())---:" + this.getUser(user.getUsername()));
                    //SlaveRedisDao slaveRedisDao = ctx.getBean("redisSlaveDao", SlaveRedisDao.class);
                    //registBack.setSessionid(slaveRedisDao.setSession());
                    //session.setAttribute(session.getId(), this.getUser(user.getUsername()));
                    UserController.cahceBuilder.put(this.getDbUser().getUsername(), this.getDbUser());
                } else {
     ...
     * @return
     */
    public UserCallBack getUserChangePwd(User user, String newpassword) {
        UserCallBack registBack = new UserCallBack(user.getUsername(), null, null,
                DateUtil.getCurrentTimeMinute(), REG_FAILED, null);
        StringBuilder result = new StringBuilder();
