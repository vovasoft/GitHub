	...
	$.ajax({
        ...
        success: function(data) {
        	switch(data.flag){
        		case 502:
        		if(data.user != null){
        			if(data.user.username=="7Graus"||data.user.username=='Spilgame'||data.user.username=="spilgamebr"||
        			data.user.username=='spilgameen'|| data.user.username=='marlonloh'){
        				isGrausFlag=true;
        			}
...
//  var path = "http://localhost:8080/manage/manage_ch?app=7001&uid="+uid+"&page=1";
...
function setData(data){
			...
			"<th style='text-align: center;' id='id_owner'></th><th style='text-align: center;' id='id_total_number_copy'></th>"+
			"<th style='text-align: center;' id='id_payed_number_copy'></th><th style='text-align: center;' id='id_income_copy'></th>"+
			"<th style='text-align: center;' id='id_status'></th></tr></thead><tbody id='tbody'></tbody>");
			}
			$.each(detailArr, function(index,item) {
	        		...
	        		switch(item.product) {
					   	...
					   	break
					   	case '5':
					   	str += "<td style='text-align: center;'>NARUTO</td>";
						 break
						 case '6':
					   	str += "<td style='text-align: center;'>HeroCraft</td>";
						 break
						 case '7':
					   	str += "<td style='text-align: center;'>League of Angels Ⅱ</td>";
					   	break
					   	 case '8':
					   	str += "<td style='text-align: center;'>Bleach X</td>";
						 break
					}
